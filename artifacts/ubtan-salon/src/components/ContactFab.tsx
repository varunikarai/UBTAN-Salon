  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3">
      <AnimatePresence>
        {open &&
          ACTIONS.map((action, i) => (
            ...
            >
              <span className="rounded-sm border border-primary/20 bg-background/90 px-3 py-1.5 text-xs uppercase tracking-[0.2em] text-foreground opacity-0 shadow-lg backdrop-blur-sm transition-opacity duration-200 group-hover:opacity-100">
                {action.label}
              </span>
              <span className="flex h-12 w-12 items-center justify-center rounded-full border border-primary/30 bg-card text-primary shadow-[0_8px_24px_rgba(0,0,0,0.35)] transition-colors duration-300 group-hover:bg-primary group-hover:text-primary-foreground">
      ...
      <motion.button
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-[0_12px_32px_rgba(212,175,55,0.35)]"
      >
        <motion.span
